...
void eloop_run(void)
{
	...
	while (!eloop.terminate &&
		...
		// readers are used for receive packet data - only one will ever be connected.
		for (int i = 0; i < eloop.readers.count; i++) {
			eloop.readers.table[i].handler(
				eloop.readers.table[i].sock,
				eloop.readers.table[i].eloop_data,
				eloop.readers.table[i].user_data);
		}

		// writers are used for sendl2 packet data - only one will ever be connected.
		for (int i = 0; i < eloop.writers.count; i++) {
			eloop.writers.table[i].handler(
				eloop.writers.table[i].sock,
				eloop.writers.table[i].eloop_data,
				eloop.writers.table[i].user_data);
		}
	}
